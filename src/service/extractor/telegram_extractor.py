...
def extract_telegram(url: str) -> str:
    ...
    for counter in channel_counters:
        counter_type = counter.find(
            "span", class_="counter_type").get_text(strip=True)
        counter_value = counter.find(
            "span", class_="counter_value").get_text(strip=True)
        channel_info[counter_type] = counter_value

    # Extracting messages
    for message in soup.find_all("div", class_="tgme_widget_message"):
        message_content = message.find(
